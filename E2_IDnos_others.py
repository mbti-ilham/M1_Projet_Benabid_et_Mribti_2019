...
list=[]
gen_id_lists=[]
with open("E1_tableau-data.txt") as f:
    next(f)
    for linef in f:
        list.append(linef)
        line = linef.split('	')
        ...
        if(line[2]=='Trans' and line[3]=='-' and i>1):
          ...
          if((line[1] == list[i-1].split('	')[1]) and (line[1] == list[i-2].split('	')[1]) and #pul_id
                (list[i-2].split('	')[2] == 'other' and list[i-1].split('	')[2] == 'SusD')):
        ...
        elif(line[2]=='other' and line[3] == '+' and i>1):
...
newfile = open("E2_table-of-IDothers.txt","a+")  
